    ...
    def post(self, request):
        ...
        if register_form.is_valid():
            ...
            if UserProfile.objects.filter(email=email):
                return render(request, "login.html", {"register_form": register_form, "msg": "用户已存在"})
...
class ModifyPwdView(View):
    ...
    def post (self, request):
        ...
        if modify_form.is_valid():
            ...
            email = request.POST.get("email", "")
            return render(request, "password_reset.html", {"email": email, "msg": "密码不符合要求"})
...
class IndexView(View):
    def get(self, request):
        ...
        banner_courses = Course.objects.order_by('-add_time').filter(is_banner=True)[:3]
